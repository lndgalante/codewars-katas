...
function eqAll(iterable) {
  ...
  if (typeof iterable === 'object' && !Array.isArray(iterable) && iterable.size > Number.MAX_SAFE_INTEGER) return false
  ...
  if (typeof iterable === 'string') {
    ...
    result = stringSplitted.every(
      (element, index, array) => (index + 1 < array.length ? element === array[index + 1] : true)
    )
  }

  if (Array.isArray(iterable)) {
    result = iterable.every((element, index, array) => (index + 1 < array.length ? element === array[index + 1] : true))
  }

  if (typeof iterable === 'object' && !Array.isArray(iterable) && iterable.size === 1) return true

  if (typeof iterable === 'object' && !Array.isArray(iterable) && iterable.size > 1) return false

  if (typeof iterable === 'object' && iterable.toString() === '[object Object]') {
...
const set3 = new Set(['d', 'g', 'o', 'q', 'r', 't', 'e', 'j', 'l', 'u', 'f', 'p'])
const set4 = new Set()
